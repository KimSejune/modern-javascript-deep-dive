...
* 정보 은닉을 통하여 `객체간의 결합도를 낮춘다`.
* JavaScript에서는 접근제한자를 제공하지 않기에 `클로저를 활용`한다.
* `JavaScript는 정보 은닉을 완벽하게 지원하지 않는다.`
  * Symbol, WeakMap을 사용하여 private을 흉내 내었다. 
  * Node 12버전 이상에서는 private이 구현되어있다.
...
즉시 실행 함수가 반환하는 Person 생성잠 함수와 Person 생성자 함수의 인스턴스가 상속받아 호출할 Person.prototype.sayHi 메서드는 즉시 실행 함수가 종료된 이후 호출된다.  
하지만 `Person 생성자 함수와 sayHi 메서드는 이미 종료되어 소멸한 즉시 실행 함수의 지역 변수 _age를 참조할 수 있는 클로저`이다.

위 코드의 문제는 `_age 변수의 상태가 유지되지 않는다는 것`이다.
...
Person.prototype.sayHi 메서드가 `단 한 번 생성되는 클로저이기 때문에 발생하는 현상`이다.
> 둘다 같은 즉시실행 함수의 Excution Context를 [[Environment]]에 저장한다.



> let 키워드로 선언한 변수를 사용하면 `for문의 코드 블록이 반복 실행될 때마다 새로운 Lexical Environment가 생성`된다.